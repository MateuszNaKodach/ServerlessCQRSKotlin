...
class Reservation : AggregateBase<Reservation.State>(State())
{
    ...
    init{
        ...
        registerTransition(CancelReservation::class, fun(state: State, event: ReservationCanceled): State {
            return apply(state, event)
        })
    }
    ...
    fun cancelReservation(command: CancelReservation): Array<IEvent> {
        raiseEvent(ReservationCanceled(command.id, command.hotelId, command.roomType))
        return unCommitedEvents.toTypedArray()
    }
    ...
    private fun apply(state: State, event: ReservationCanceled): State {
        state.id = event.id
        state.hotelId = event.hotelId
